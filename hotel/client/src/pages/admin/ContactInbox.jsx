  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl border border-border p-6 shadow-sm">
        <h1 className="text-2xl font-serif-display font-medium text-primary">Hộp thư liên hệ</h1>
        ...
        <div className="space-y-3">
          {messages.map((m) => (
            <div key={m._id} className={`bg-white rounded-xl border shadow-sm p-5 transition-shadow hover:shadow-md ${!m.isRead ? 'border-l-4 border-l-accent border-r-border border-y-border' : 'border-border'}`}>
              ...
              <p className="mt-3 text-slate-600 whitespace-pre-wrap text-sm leading-relaxed bg-[#FAF9F6] border border-border rounded-lg p-4 font-light">{m.message}</p>
