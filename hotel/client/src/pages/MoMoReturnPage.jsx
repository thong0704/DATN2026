  ...
  useEffect(() => {
    const verify = async () => {
      try {
        ...
        const res = await fetch(`${API_URL}/payments/momo-return?${queryString}`);
    ...
    <div className="max-w-xl mx-auto py-12 text-center card p-8">
      {status === 'success' ? (
        <>
          ...
          <p className="text-gray-600 mb-6">Giao dịch MoMo đã được xác nhận.</p>
        ...
        <>
          ...
          <p className="text-gray-600 mb-6">Giao dịch MoMo không thành công hoặc đã bị huỷ.</p>
          {bookingId && (
